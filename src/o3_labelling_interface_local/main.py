...
@st.cache_resource
def get_images_from_folder(thumbnails_parent_dir:str, video_id:str, n:int=10) -> tuple[list[int], list[str]]:
    """
    ...
    image_paths = os.listdir(f'{thumbnails_parent_dir}/{video_id}')
    image_paths.sort()

    for i, filename in enumerate(image_paths):

        if i % n == 0: 
            with open(f'{thumbnails_parent_dir}/{video_id}/{filename}', 'rb') as file:
                image_data = base64.b64encode(file.read()).decode('utf-8')
                filename_array.append(f'{thumbnails_parent_dir}/{video_id}/{filename}')
...
def preload_images(thumbnails_parent_dir:str, video_ids_to_preload:list[str], step:int) -> None:
    """
    ...
    for i, video_id in enumerate(video_ids_to_preload):
        try:
            st.session_state[f"{video_id}_4"] = get_images_from_folder(thumbnails_parent_dir=thumbnails_parent_dir, video_id=video_id, n=step)
        except:
...
def switch_to_thumbnails_page(video_nb:int) -> None:
    ...
    st.session_state["thumbnails_parent_dir"] = thumbnails_parent_dir
    st.session_state["results_parent_dir"] = results_parent_dir
    st.session_state["rendered_frames_per_second"] = rendered_frames_per_second
...
#              Global Parameters              #
###############################################
csv_filepath = "../../data/jan_videos_summary.csv"
thumbnails_parent_dir = "../../data/thumbnails/thumbnails_60secsPerFrame_160px120px"
results_parent_dir = "../../data/labelling_results/thumbnails_60secsPerFrame_160px120px"
rendered_frames_per_second = 60
...
thumbnails_dir_list = os.listdir(thumbnails_parent_dir)
results_dir_list    = os.listdir(results_parent_dir)

results_video_id_list = [x.split('.')[0] for x in results_dir_list]
st.text(thumbnails_dir_list)
...
step_slider = st.number_input(f"Thumbnails were generated for every {rendered_frames_per_second} seconds of the video. Preload every n-th thumbnail", 
                              value=4, 
                              min_value=1, 
                              placeholder=4)
st.text(f'Preloading one thumbnail every {int(rendered_frames_per_second*step_slider)} seconds.')
st.text('')
st.text('')
...
if st.button("Preload") and len(video_ids_to_preload) > 0:
    preload_images(thumbnails_parent_dir, video_ids_to_preload, step_slider)
    st.rerun()
...
try:
    ...
    current_labelling_classes = "Interlude"
