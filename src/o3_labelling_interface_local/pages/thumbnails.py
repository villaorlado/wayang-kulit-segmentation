...
from main import get_images_from_folder
import re
import json
...
def write_results(video_id:str, labelling_resolution:int, select_boxes:dict) -> None:
    """
    Save the results to the server

    [Arguments]
    video_id            - YouTube video ID of the video.
    thumbnail_view_size - resolution of labelling (rendered_frames_per_second * step_val)
    ...
    json_data = {
		"labelling_resolution": labelling_resolution,
		"data": labelled_intervals_dict
	}

    with open(f"{results_parent_dir}/{video_id}.json", "w") as remote_file:
...
def get_previous_select_boxes_result(video_id:str) -> dict:
    ...
    try:
        with open(f"{st.session_state["results_parent_dir"]}/{video_id}.json", "r") as result_file:
...
def select_boxes_to_intervals(select_boxes:dict) -> list[tuple[int,int]]:
    ...
    # Grab the largest frame number
    max_number = max(
        select_boxes.keys(), 
        key=lambda x: int(os.path.basename(x).split('.')[0])
    )
    
    largest_frame_nb = int(os.path.basename(max_number).split('.')[0])
    ...
    for frame_name, labels in select_boxes.items():
        if labels != []:
            ...
            for label in labels:
                ...
                frame_nb = int(os.path.basename(frame_name).split('.')[0])
...
def select_interlude_intervals(video_id:str, filename_array:list[str]) -> dict:
    ...
    for label, intervals in stored_file.items():
        ...
        for filename in filename_array:
            ...
            # Initialise
            frame_nb = int(os.path.basename(filename).split('.')[0])
...
def get_preloaded_images(video_id:str, step:int) -> tuple[list[str], list[str]]:
    ...
    if key not in st.session_state:
        st.session_state[key] = get_images_from_folder(thumbnails_parent_dir=thumbnails_parent_dir, video_id=video_id, n=step)
...
if 'video_id' in st.session_state:
    ...
    thumbnails_parent_dir = st.session_state["thumbnails_parent_dir"]
    results_parent_dir    = st.session_state["results_parent_dir"]
    rendered_frames_per_second = st.session_state["rendered_frames_per_second"]
else:
...
nb_thumbnails = int(nb_frames/(fps * rendered_frames_per_second)) + 1
...
st.text(f'Thumbnails were generated for every {rendered_frames_per_second} seconds of the video.')
step_slider = st.number_input("Grab every n-th thumbnail", value=step_val, min_value=1, placeholder=step_val)
st.text(f'Displaying one thumbnail every {int(rendered_frames_per_second*step_slider)} seconds. Total of {int(nb_thumbnails/step_slider) + 1} thumbnails.')
...
if image_data_array != []:
    ...
    if previous_result_exists == False or (previous_result_exists == True and overwrite == True):
        if st.button('Submit labelling results to server'):
            write_results(video_id, step_val*rendered_frames_per_second, select_boxes)  
